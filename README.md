...
- **ドライバ** … 純正 `alps,ec11` ではなく、このリポジトリ内の
  `drivers/sensor/detent_encoder` (ノッチ同期型) を使っている。純正は遷移を
  1つ取りこぼすと端数がノッチ位置からずれたまま残り、回し始めや逆回転時に
  ときどき空振りしていた。こちらは A/B が静止位置へ戻るたびに1ノッチと数えて
  位相を合わせ直すので、ずれが残らない。1ノッチ = 1トリガの原則はそのまま。
  デコード部分は `tests/detent_decoder_test.c` でホスト上で検証できる
