...
- **既知の制約** … 回し始めの1ノッチだけ空振りすることがある (回し続けている
  間は出ない)。ZMK の EC11 ドライバは割り込み処理中に自分の割り込みを止め、
  その間に2遷移入るとデコード表が `default: delta = 0` で黙って捨てるため、
  半ノッチ分がずれる。`CONFIG_EC11_TRIGGER_OWN_THREAD` で緩和を試したが
  効果が無かった (経緯は `boards/shields/roBa/roBa_L.conf` のコメント参照)。
  直すにはドライバのフォークが要る
